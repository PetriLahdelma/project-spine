...
**Acme Payroll Marketing Site** — Launch a marketing site for Acme Payroll's new SMB product line within 6 weeks.

**Stack:** next • typescript • tailwind • tests: vitest

**Project type:** `saas-marketing`

> Source of truth: `.project-spine/spine.json` (hash `f2c638c046f2ee9c`). Every rule below has a traceable source. If a rule is wrong, fix the upstream input (`brief.md`, repo state, or `design-rules.md`) and rerun `spine compile`.
